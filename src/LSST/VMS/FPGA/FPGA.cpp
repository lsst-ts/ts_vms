...
int32_t FPGA::open() {
  Log.Debug("FPGA: open()");
  int32_t status =
      NiFpga_Open(this->bitFile, this->signature, "RIO0", 0, &(this->session));
...
int32_t FPGA::writeCommandFIFO(uint16_t *data, int32_t length,
                               int32_t timeoutInMs) {
  Log.Trace("FPGA: writeCommandFIFO(%d)", length);
  return NiFpga_WriteFifoU16(this->session, this->commandFIFO, data, length,
                             timeoutInMs, &this->remaining);
}

int32_t FPGA::writeRequestFIFO(uint16_t *data, int32_t length,
                               int32_t timeoutInMs) {
  Log.Trace("FPGA: writeRequestFIFO(Length = %d)", length);
  return NiFpga_WriteFifoU16(this->session, this->requestFIFO, data, length,
                             timeoutInMs, &this->remaining);
}
...
int32_t FPGA::readU64ResponseFIFO(uint64_t *data, int32_t length,
                                  int32_t timeoutInMs) {
  Log.Trace("FPGA: readU64ResponseFIFO(%d)", length);
  return NiFpga_ReadFifoU64(this->session, this->u64ResponseFIFO, data, length,
                            timeoutInMs, &this->remaining);
}

int32_t FPGA::readSGLResponseFIFO(float *data, int32_t length,
                                  int32_t timeoutInMs) {
  Log.Trace("FPGA: readSGLResponseFIFO(%d)", length);
  return NiFpga_ReadFifoSgl(this->session, this->sglResponseFIFO, data, length,
                            timeoutInMs, &this->remaining);
}
