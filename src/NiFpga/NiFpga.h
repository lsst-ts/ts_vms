 ...
 * another method for I/O or
 * change the controller associated with the FPGA
 * target.
 */
 ...
 * when the combined depth
 * of all DMA FIFOs exceeds the maximum depth for the
 * controller, or when the controller runs out of system
 * memory. You may be able
 ...
 * Close the other session and retry this operation.
 */
 ...
 * target, use MAX to
 * check the Remote Device Access settings under
 * Software>>[Recommended software set, such as "NI
 * CompactRIO version -
 ...
 * /etc/natinst/nirio/nirio.ini on the Linux
 * target.
 */
 ...
 * unavailable, either
 * because it was disconnected from the network or because
 * power was removed (e.g., the system went into a
 * low-power state). Close any
 * RIO FPGA sessions; check network, USB, and power connections; power-cycle
 * the
 ...
 * target. Upgrade the
 * NI-RIO software on the host in order to connect to this
 * target.
 */
 ...
 * and accessible over the network, that the device is not
 * in safe mode, that NI-RIO software is installed,
 * and that the RIO server is
 ...
 * remote device. Retry the
 * operation. If the remote device continues to report
 * this error, check its power supply and look for
 ...
 * have timed out because of processor overuse. Check the
 * network connection, reduce the demand on the
 * processor, and decrease the
 ...
 * NI-RIO software is
 * installed and that the RIO server is running and properly
 * configured.  Refer to Software>>NI-RIO>>NI-RIO
 * Settings under the system in
 * MAX.
 */
 ...
 * occurs when the resource string is specified as
 * rio://hostname/device_name.  A local session can be
 * opened by specifying just
 * the device_name.
 */
 ...
 * settings or the trigger
 * reservations for each device within the system.
 */
 ...
 * Wait on IRQ after an
 * Abort causes this error.
 */
 ...
 * milliseconds. Extend the time
 * period, or ignore if the result was expected.
 */
 ...
 * while the host had
 * acquired elements of the FIFO. Release all acquired
 * elements before configuring, stopping, reading, or
 * writing.
 */
 ...
 * maximum size supported on the current target. Refer to
 * the hardware documentation for the limitations on
 * data types for this target.
 */
 ...
 * the correct function for the property.
 */
 ...
 * bitfile, the bitfile may not
 * be compatible with the software you are using.
 * Determine which version of LabVIEW was used to make the
 * bitfile, update your
 ...
 * the original bitfile or
 * regenerate the C API for this bitfile.
 */
 ...
 * and/or host. Update the version of NI-RIO on the
 * target and/or host to the same version (or later) used
 * to compile the
 * bitfile. Alternatively, recompile the bitfile with the same version of NI-RIO
 * that is
 * currently installed on the target and/or host.
 */
 ...
 * you can set when the
 * bitfile is autoloaded. Download the bitfile to the flash
 ...
 * and reboot the system. To avoid this error, monitor
 * the power consumption, power shutdown threshold, and
 * margin of the device. If
 ...
 * airflow and cooling
 * and reboot the system. To avoid this error, monitor the
 * FPGA temperature, temperature shutdown
 * threshold, and margin of the FPGA. If
 ...
 * conditions:  1) Cannot be or
 * contain a reserved word. 2) Can use only
 * alphanumeric characters, hyphens, and underscores. 3) Cannot
 * conflict with
 * the default alias of another device. A default alias is the name that the NI-
 * RIO
 * Device Driver assigns to a device based on its attributes if you have not
 * assigned a custom alias to the
 * device using MAX or the System Configuration
 * API. For example, 'PXI1Slot3' is the default alias for the
 * device at Slot 3
 * of PXI Chassis 1, and it cannot be used as an alias name for any other
 * device. Refer
 * to the LabVIEW Help for NI-RIO for more information about
 * aliases.
 */
 ...
 * 65535, where 0 indicates a
 * dynamically assigned port. Port 3580 is reserved
 * and cannot be used.
 */
 ...
 * invalid. Use MAX to
 * install all necessary software to your target. Note that
 * if your project includes modules under the
 * chassis item, user-defined
 * variables, or references to the Scan Clock Chassis I/O item, you must
 * install
 ...
 * bitfile is already open on
 * the device. Close the current session before
 * opening another.
 */
 ...
 */
static NiFpga_Inline float NiFpga_CalculateFxpDeltaFloat(const NiFpga_FxpTypeInfo typeInfo) {
...
#define NiFpga_Private_FxpToFloatingPoint(typeInfo, delta, data)                                   \
    const uint64_t wordLengthMask =                                                                \
            typeInfo.wordLength != 64 ? (1ULL << typeInfo.wordLength) - 1 : 0xFFFFFFFFFFFFFFFFULL; \
    data &= wordLengthMask;                                                                        \
 ...
 */
static NiFpga_Inline float NiFpga_ConvertFromFxpToFloat(const NiFpga_FxpTypeInfo typeInfo, uint64_t data) {
 ...
 * @return delta the delta for the specified FXP type
 */
static NiFpga_Inline double NiFpga_CalculateFxpDeltaDouble(const NiFpga_FxpTypeInfo typeInfo) {
    const int32_t exponent = typeInfo.integerWordLength - typeInfo.wordLength;
 ...
 */
static NiFpga_Inline double NiFpga_ConvertFromFxpToDouble(const NiFpga_FxpTypeInfo typeInfo, uint64_t data) {
...
#define NiFpga_Private_FloatingPointToFxp(typeInfo, delta, data)                                   \
    ...
    if (data < 0) {                                                                                \
        if (typeInfo.isSigned) {                                                                   \
            ...
            {                                                                                      \
                return (-1LL * (1LL << (typeInfo.wordLength - 1))) & wordLengthMask;               \
            }                                                                                      \
        } else {                                                                                   \
            return 0;                                                                              \
        }                                                                                          \
        ...
        {                                                                                          \
            const uint64_t magnitude = typeInfo.wordLength - (typeInfo.isSigned ? 1 : 0);          \
 ...
 */
static NiFpga_Inline uint64_t NiFpga_ConvertFromFloatToFxp(const NiFpga_FxpTypeInfo typeInfo, float data) {
    const float delta = NiFpga_CalculateFxpDeltaFloat(typeInfo);
 ...
 * @return a value containing the FXP representation of data
 */
static NiFpga_Inline uint64_t NiFpga_ConvertFromDoubleToFxp(const NiFpga_FxpTypeInfo typeInfo, double data) {
    const double delta = NiFpga_CalculateFxpDeltaDouble(typeInfo);
    NiFpga_Private_FloatingPointToFxp(typeInfo, delta, data)
}
 ...
 * @return result of the call
 */
NiFpga_Status NiFpga_GetBitfileSignature(NiFpga_Session session, uint32_t* signature, size_t* signatureSize);
 ...
 * @return result of the call
 */
NiFpga_Status NiFpga_FindRegister(NiFpga_Session session, const char* registerName, uint32_t* registerOffset);
 ...
 * @return result of the call
 */
NiFpga_Status NiFpga_FindFifo(NiFpga_Session session, const char* fifoName, uint32_t* fifoNumber);
 ...
 * @return result of the call
 */
NiFpga_Status NiFpga_ReadFxp64(NiFpga_Session session, uint32_t indicator, NiFpga_FxpTypeInfo typeInfo,
                               uint64_t* value);
 ...
 * @return result of the call
 */
NiFpga_Status NiFpga_WriteFxp64(NiFpga_Session session, uint32_t control, NiFpga_FxpTypeInfo typeInfo,
                                uint64_t value);
...
#else
typedef enum { NiFpga_FifoProperty_NotSupported } NiFpga_FifoProperty;
 ...
 * @return result of the call
 */
NiFpga_Status NiFpga_UnreserveFifo(NiFpga_Session session, uint32_t fifo);
 ...
 * @return result of the call
 */
NiFpga_Status NiFpga_MapP2PSinkFifo(const NiFpga_Session session, const uint32_t fifo, size_t* size,
                                    void** virtualAddress);
 ...
 * @param fifo peer-to-peer Sink FIFO
 */
NiFpga_Status NiFpga_UnmapP2PSinkFifo(const NiFpga_Session session, const uint32_t fifo);
 ...
 * @return result of the call
 */
NiFpga_Status NiFpga_OpenHostMemoryBuffer(const NiFpga_Session session, const char* memoryName,
                                          size_t* memorySize, void** virtualAddress);
 ...
 * @param memoryName Name given to the memory in the FPGA target
 */
NiFpga_Status NiFpga_CloseHostMemoryBuffer(const NiFpga_Session session, const char* memoryName);
 ...
 * @return result of the call
 */
NiFpga_Status NiFpga_OpenLowLatencyBuffer(const NiFpga_Session session, const char* memoryName,
                                          size_t* memorySizeToHost, void** virtualAddressToHost,
                                          size_t* memorySizeToFpga, void** virtualAddressToFpga);
 ...
 * @param memoryName Name given to the memory in the FPGA target
 */
NiFpga_Status NiFpga_CloseLowLatencyBuffer(const NiFpga_Session session, const char* memoryName);
